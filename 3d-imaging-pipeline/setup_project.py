...
def run(project_name):
    ...
    print(f'Generating cd /dfolders for project {project_name}')
    ...
    seg_cfg['ProbabilitiesFileName'] = f'{project_name}_probabilities.h5'
    seg_cfg['PredictionFileName'] = f'{project_name}_prediction.h5'
    ...
    ext_cfg = yaml_obj.load(config_path / Path("hpc_data_extraction_template.yaml"))
    ext_cfg['ProjectName'] = project_name
    ext_cfg['ProjectPath'] = str(project_dir)
    ext_cfg['PredictionFileName'] = f'{project_name}_prediction.h5'
...
cd {root_path}
python -m run_segmentation "{out_seg_cfg.relative_to(root_path)}"
''')
...
conda activate {os.environ['CONDA_DEFAULT_ENV']}
cd {root_path}
python -m run_extraction "{out_ext_cfg.relative_to(root_path)}"
''')
